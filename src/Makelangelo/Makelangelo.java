		...
		implements ActionListener, KeyListener, SerialPortEventListener
{
	...
	private JMenuItem buttonHilbertCurve, buttonText2GCODE, buttonSaveFile, buttonSketch;
    ...
    private JMenuItem buttonStart, buttonStartAt, menuButtonPause, buttonHalt;
	...
	private AbstractButton buttonPause;
	...
	protected void LoadImageConverters() {
		...
		image_converters[i++] = new Filter_GeneratorTSP_Pen();
	...
	/*private void DrawSketch() {
		//loadPane.resize(true);
		CardLayout cardLayout = (CardLayout) loadnpreview.getLayout();
		cardLayout.next(loadnpreview);	
	}*/
	...
	public void actionPerformed(ActionEvent e) {
		...
		if( subject == buttonRun ) {
			if(gcode.fileOpened && !running) {
				gcode.linesProcessed=1;
				SendLineToRobot("M110 N"+gcode.linesProcessed);
				previewPane.setLinesProcessed(gcode.linesProcessed);
				StartDrawing();
			}
...
;
		...
		if( subject == menuButtonPause ) {
			if(running) {
				if(paused==true) {
					penIsUpBeforePause=penIsUp;
					RaisePen();
					menuButtonPause.setText(MultilingualSupport.getSingleton().get("Pause"));
					paused=false;
					...
					menuButtonPause.setText(MultilingualSupport.getSingleton().get("Unpause"));
					paused=true;
				}
			}
			return;
	...
	public void UpdateMenuBar() {
        ...
        buttonSketch = new JMenuItem(MultilingualSupport.getSingleton().get("Draw Sketch"));
        buttonSketch.addActionListener(this);
        menu.add(buttonSketch);
        ...
        menuButtonPause = new JMenuItem(MultilingualSupport.getSingleton().get("Pause"),KeyEvent.VK_P);
        menuButtonPause.addActionListener(this);
        menuButtonPause.setEnabled(portConfirmed && running);
        menu.add(menuButtonPause);
    ...
    public Container CreateContentPane() {
        ...
        loadPane = new JPanel();
        buttonOpenFile = new JButton("Select File");
        buttonOpenFile.addActionListener(this);
        buttonRun = new JButton("Print This Image");
        buttonRun.addActionListener(this);
        buttonPause = new JButton("Pause");
        buttonPause.addActionListener(this);
        loadPane.add(buttonOpenFile);
        loadPane.add(buttonRun);
        loadPane.add(buttonPause);
        Splitter loadnpreview = new Splitter(JSplitPane.VERTICAL_SPLIT);
        loadnpreview.setResizeWeight(1.0);
        loadnpreview.add(previewPane);
        loadnpreview.add(loadPane);
        ...
        //loadnpreview.setDividerLocation(contentPane.getHeight()-buttonOpenFile.getHeight());
        return contentPane;
    }
