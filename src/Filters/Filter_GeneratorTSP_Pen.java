 ...
 * @author Dan
 */
public class Filter_GeneratorTSP_Pen extends Filter {
	public String GetName() { return "Big zig zag w/ Pen Lift"; }
...
/*
	...
	private void MoveTo(OutputStreamWriter out,int i,boolean up) throws IOException {
		...
		if (i==0)
			prevIndex = numPoints;
		float x0 = points[solution[prevIndex]].x;
		float y0 = points[solution[prevIndex]].y;
		float x1 = points[solution[i]].x;
		float y1 = points[solution[i]].y;
		double dist = Math.sqrt(Math.pow((x0-x1), 2)+Math.pow((y0-y1), 2));
		
		log.println(String.valueOf(dist));
		boolean isLong = dist > 4;
		if (isLong) {
			liftPen(out);
		}
